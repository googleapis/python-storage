...
async def download_one_async(client, bucket_name, object_name, download_size, chunk_size):
    """
    Downloads a single object of size `download_size`, in chunks of `chunk_size`.
    Accepts an existing client object.
    """
    print(
        f"Downloading {object_name} of size {download_size} in chunks of {chunk_size} from {bucket_name} from process {os.getpid()} and thread {threading.get_ident()}"
    )
    ...
    ), f"downloaded size incorrect for {object_name}"

    await mrd.close()


def download_one_sync(bucket_name, object_name, download_size, chunk_size):
    """Wrapper to create a client and run the async download in a new event loop."""

    async def _async_wrapper():
        """Creates client and calls the download function."""
        client = AsyncGrpcClient().grpc_client
        await download_one_async(
            client, bucket_name, object_name, download_size, chunk_size
        )
...
def main():
    ...
    print(
        f"Total data to download: {total_downloaded_bytes / (1024*1024*1024):.2f} GiB"
    )
