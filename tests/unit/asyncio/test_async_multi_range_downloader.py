...
2. read_ranges length - 0, 1, 3, 45, 101, 500, 1000, 1001
   ranges downloaded should be
   * 1KiB
   10Kib
   1 MiB
   10 Mib
...
4. memory/ cpu used by the application
"""
...
1. create mRD
   download ranges , 1 -range, .. n ranges


   success, not error obj
   assert on data ?
       less than 10 bytes.


       larger bytes should always be on system-tests.s
...
failure
1. if > 1000 ranges  , fail
   create error obj, code details


2. type.googleapis.com/google.storage.v2.BidiReadObjectError => raise error
   assert on error_obj , code , details


3. how to simulate connection interrupt ?
   after n response , (n+1)th response should raise error.
   assert on error_obj , code , details, [results should be partial]


   * actual test ?


   * storage test-bench ? tests conformation tests ?
