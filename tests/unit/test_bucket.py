...
class Test_Bucket(unittest.TestCase):
    ...
    def test_upload_many_by_filenames(self):
        from google.cloud.storage.blob import Blob

        bucket = self._make_one()
        FAKE_ROOT = "fake_assets/"
        FAKE_FILENAMES = ["fake_a.txt", "fake_b.txt"]
        FAKE_PREFIX = "my_assets/"
        CHUNK_SIZE = 1024 * 1024
        BLOB_CONSTRUCTOR_KWARGS = {"chunk_size": CHUNK_SIZE}
        FAKE_CONTENT_TYPE = "text/fake"
        UPLOAD_KWARGS = {"content-type": FAKE_CONTENT_TYPE}
        MAX_WORKERS = 99

        with mock.patch.object(Blob, "upload_many", autospec=True) as many_mock:
            DESIRED_RETURN_VALUE = "return_string"
            many_mock.return_value = DESIRED_RETURN_VALUE
            result = bucket.upload_many_from_filenames(
                FAKE_FILENAMES,
                FAKE_ROOT,
                prefix=FAKE_PREFIX,
                skip_if_exists=True,
            ...
            self.assertEqual(result, DESIRED_RETURN_VALUE)
            ...
            pairs = many_mock.call_args[0][0]
            for index, (path, blob) in enumerate(pairs):
                expected_path = FAKE_ROOT + FAKE_FILENAMES[index]
                expected_blob_name = FAKE_PREFIX + FAKE_FILENAMES[index]
                self.assertEqual(path, expected_path)
                self.assertEqual(blob.bucket, bucket)
                self.assertEqual(blob.name, expected_blob_name)
                self.assertEqual(blob.chunk_size, CHUNK_SIZE)
    ...
    def test_download_many_to_path(self):
        ...
        FAKE_PREFIX = "my_assets/"
        BLOBS = [
            bucket.blob(FAKE_PREFIX + FAKE_FILENAMES[0]),
            bucket.blob(FAKE_PREFIX + FAKE_FILENAMES[1])
        ]
        FAKE_ROOT = "fake_assets/"
        DOWNLOAD_KWARGS = {"raw_download": True}
        MAX_WORKERS = 99

        with mock.patch.object(Blob, "download_many", autospec=True) as many_mock:
            DESIRED_RETURN_VALUE = "return_string"
            many_mock.return_value = DESIRED_RETURN_VALUE
            result = bucket.download_many_to_path(
                BLOBS,
                FAKE_ROOT,
            ...
            pairs = many_mock.call_args[0][0]
            for index, (blob, path) in enumerate(pairs):
                expected_path = FAKE_ROOT + FAKE_FILENAMES[index]
                expected_blob_name = FAKE_PREFIX + FAKE_FILENAMES[index]
                self.assertEqual(path, expected_path)
                self.assertEqual(blob.bucket, bucket)
    ...
    def test_download_many_to_path_strip_prefix_error(self):
        from google.cloud.storage.blob import Blob

        bucket = self._make_one()
        FAKE_FILENAMES = ["fake_a.txt", "fake_b.txt"]
        FAKE_PREFIX = "my_assets/"
        WRONG_PREFIX = "their_assets/"
        BLOBS = [
            bucket.blob(FAKE_PREFIX + FAKE_FILENAMES[0]),
            bucket.blob(WRONG_PREFIX + FAKE_FILENAMES[1])
        ]
        FAKE_ROOT = "fake_assets/"
