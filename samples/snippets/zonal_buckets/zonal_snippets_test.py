...
def test_storage_create_and_write_appendable_object(async_grpc_client, json_client, event_loop, capsys):
...
def test_storage_finalize_appendable_object_upload(async_grpc_client, json_client, event_loop, capsys):
...
def test_storage_pause_and_resume_appendable_upload(async_grpc_client, json_client, event_loop, capsys):
...
def test_storage_read_appendable_object_tail(async_grpc_client, json_client, event_loop, capsys):
...
def test_storage_open_object_read_full_object(async_grpc_client, json_client, event_loop, capsys):
    object_name = f"test-open-read-full-{uuid.uuid4()}"
    data = b"Hello, is it me you're looking for?"   
    event_loop.run_until_complete(create_appendable_object(async_grpc_client, object_name, data))
    event_loop.run_until_complete(
    ...
    out, _ = capsys.readouterr()
    assert f"Downloaded all {len(data)} bytes from object {object_name} in bucket {_ZONAL_BUCKET}." in out
    blob = json_client.bucket(_ZONAL_BUCKET).blob(object_name)
    blob.delete()


def test_storage_open_object_single_ranged_read(async_grpc_client, json_client, event_loop, capsys):
    object_name = f"test-open-single-range-{uuid.uuid4()}"
    event_loop.run_until_complete(create_appendable_object(async_grpc_client, object_name, b"Hello, is it me you're looking for?"))
    download_size = 5
    event_loop.run_until_complete(
        storage_open_object_single_ranged_read.storage_open_object_single_ranged_read(
            _ZONAL_BUCKET, object_name, start_byte=0, size=download_size, grpc_client=async_grpc_client
        )
    )
    out, _ = capsys.readouterr()
...
def test_storage_open_object_multiple_ranged_read(async_grpc_client, json_client, event_loop, capsys):
...
def test_storage_open_multiple_objects_ranged_read(async_grpc_client, json_client, event_loop, capsys):
    ...
    event_loop.run_until_complete(create_appendable_object(async_grpc_client, blob1_name, data1))
    event_loop.run_until_complete(create_appendable_object(async_grpc_client, blob2_name, data2))
